...
import { getUserToken, recordSwipe, subscribeToSwipes } from '../lib/room'
...
// Fallback demo restaurants when no API key or location fails
...
export default function FoodRoom({ room, onDone }) {
  const [phase, setPhase] = useState('cuisine') // cuisine | locating | restaurant | matched | alldone
  ...
  // Keep ref in sync so subscription closure sees fresh list
  useEffect(() => { restaurantsRef.current = restaurants }, [restaurants])

  // Subscribe to matches from partner
  useEffect(() => {
  ...
  function handleCuisineMatch(cuisine) {
    setMatchedCuisine(cuisine)
    setPhase('locating')
    fetchRestaurants(cuisine.title)
  }

  function handleRestaurantMatch(restaurant) {
  ...
  async function fetchRestaurants(cuisineTitle) {
    if (!GOOGLE_KEY) {
      setLocationNote('No API key set — showing demo restaurants. Add VITE_GOOGLE_PLACES_API_KEY to enable real spots.')
  ...
  const handleCuisineSwipe = useCallback(async (direction) => {
    ...
    try {
      ...
      if (isMatch && direction === 'right') handleCuisineMatch(cuisine)
    } catch (err) { console.error(err) }
    setCuisineIndex(i => i + 1)
  }, [cuisineIndex, room.id]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleRestaurantSwipe = useCallback(async (direction) => {
    ...
    try {
      ...
      if (isMatch && direction === 'right') handleRestaurantMatch(restaurant)
    } catch (err) { console.error(err) }
    setRestaurantIndex(i => i + 1)
  }, [restaurantIndex, room.id]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  if (phase === 'cuisine') {
    ...
    if (!current || cuisineIndex >= CUISINES.length) {
      return (
        <div className="food-center">
          <div className="food-done-all">
            <div className="food-done-icon">😅</div>
            <h2>No matches yet</h2>
            <p>You swiped through all cuisines without a match. Try again!</p>
            <button className="btn btn-primary" onClick={() => { setCuisineIndex(0); setSwipeCount(0) }}>Start over</button>
    ...
    return (
      <div className="food-center">
        <div className="food-locating">
          ...
          <div className="food-locating-dots">
            <span /><span /><span />
          </div>
        </div>
      </div>
  ...
  if (phase === 'restaurant') {
    ...
    if (!current || restaurantIndex >= restaurants.length) {
      return (
        <div className="food-center">
          <div className="food-done-all">
            <div className="food-done-icon">🤷</div>
            ...
            <button className="btn btn-primary" onClick={() => { setPhase('cuisine'); setCuisineIndex(0); setMatchedCuisine(null) }}>Try another cuisine</button>
            <button className="btn btn-secondary" style={{ marginTop: 8 }} onClick={onDone}>Go home</button>
          </div>
        </div>
      )
    ...
    return (
      <div className="food-room">
        {locationNote && (
          <div className="food-location-note">{locationNote}</div>
        )}
        <div className="food-header">
        ...
        <div className="food-matched-modal">
          ...
          <div className="food-matched-card">
            {finalMatch.poster ? (
              ...
              <div className="food-matched-img food-matched-placeholder">
                {finalMatch.emoji || '🍽️'}
              </div>
            )}
